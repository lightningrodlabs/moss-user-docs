# First Launch

## What happens on first launch

1. **Moss starts its local conductor.** Behind the scenes Moss launches a Holochain conductor and a keystore (lair) on your machine. These run as long as Moss is open.
2. **You land on the home screen.** From here you can create your first group, or join a group using an invite link someone has sent you.

![The Moss home screen](/screenshots/home-screen.png)

## Where your data lives

Moss stores its data in your user config directory:

- **macOS:** `~/Library/Application Support/Moss/`
- **Linux:** `~/.config/Moss/`
- **Windows:** `%APPDATA%\Moss\`

## Next step

- [Create your first group](../groups/creating-a-group), or
- Accept an invite from someone else (see [Inviting Members](../groups/inviting-members) for how invites work).
